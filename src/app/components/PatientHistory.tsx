...
const PatientHistory: React.FC = () => {
    ...
    const getProfileData = (item: HistoryItem): ProfileData => {
      ...
      if ((!item.name || !item.symptom) && item.profile) {
        try {
          ...
          if (typeof item.profile === 'string' && item.profile.includes('.json')) {
            if (!item.name) {
              name = "Patient Data";
            }
            if (!item.symptom) {
  ...
  return (
    <div className="p-6 space-y-4 bg-[#0f172a] min-h-screen">
      <h2 className="text-2xl font-bold text-red-400 mb-4">Patient Chat History</h2>
      <div className="space-y-4">
        {historyItems.map((item) => {
          ...
          return (
            <div key={item.case_id} className="flex items-start justify-between p-6 border rounded-lg shadow-md bg-white">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <p className="text-lg font-semibold">
                    <span className="font-bold">Patient Name:</span> {profileData.name}
                  </p>
                  <p className="text-sm">
                    <span className="font-bold text-red-500">Symptoms:</span> {profileData.symptoms}
                  </p>
                  <p className="text-sm">
              ...
              <div className="flex space-x-4">
                <button onClick={() => handleViewAnswer(item.case_id)} className="text-blue-600 hover:underline">
                  View Answer
                </button>
                <button onClick={() => handleViewReport(item.case_id)} className="text-red-600 hover:underline">
                  Report
                </button>
              </div>
        </div>
