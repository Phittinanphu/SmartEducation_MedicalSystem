    ...
    async function runTests() {
      ...
      try {
        ...
        results.push({
          ...
          message: googleTableExists
            ? "Google accounts table exists in userdata schema"
            : "Google accounts table does not exist in userdata schema",
        });
      } catch (error) {
        results.push({
          name: "Google Accounts Table",
      ...
      if (session?.user?.email) {
        try {
          ...
          results.push({
            ...
            success: !!googleData,
            message: googleData
              ? "Found Google account data"
              : "No Google account data found for this user",
            details: (googleData as Record<string, unknown>) || undefined,
          });
        } catch (error) {
          results.push({
            name: "Google Account Data",
      ...
      <div className="w-full max-w-xl bg-white rounded-lg shadow-lg p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Testing the implementation of Google authentication
          </p>
        </div>
