...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../app/db/migrations/add_uuid_to_google_accounts.sql');
    ...
    const users = await client.query('SELECT google_id, email, uid FROM userdata.google_accounts LIMIT 5');
    ...
    if (error.message.includes('relation "userdata.google_accounts" does not exist')) {
      console.error('\nThe userdata.google_accounts table does not exist.');
      console.error('Make sure you have created the table before running this migration.');
    }
  } finally {
    client.release();
    await pool.end();
